...
function About() {
  ...
  return (
    <div className="w-screen min-h-screen bg-gradient-to-b from-[#1e1f29] via-[#2e2f3a] to-[#1e1f29] flex flex-col items-center pt-24 sm:pt-20 pb-10 px-4 font-mono">
      ...
      {/* Terminal Header */}
      <div
        className={`bg-[#1c1c1e] border border-gray-700 rounded-t-xl p-4 shadow-xl w-full max-w-4xl transform transition-all duration-700 ${
      ...
      {/* Terminal Body */}
      <div
        className={`bg-[#1c1c1e] border border-gray-700 border-t-0 rounded-b-xl p-4 sm:p-6 shadow-xl w-full max-w-4xl text-gray-300 transition-all duration-700 hover:shadow-2xl hover:scale-[1.01] hover:border-green-400 ${
          isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
        }`}
      ...
      >
        ...
        {/* Terminal Footer */}
        <div
          className={`mt-6 text-green-400 transition-all duration-700 ${
            isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
          }`}
...
function HoverBlock({ title, text, delay, isVisible }) {
  return (
    ...
    >
      <div className="text-green-300 mb-2 text-sm sm:text-base transition-colors group-hover:text-green-400">
        <span className="text-gray-400">$ </span>{title}
      </div>
      <p className="ml-2 sm:ml-4 text-xs sm:text-sm transition-colors duration-200 group-hover:text-white">
